              ...
              <div className="mt-6 bg-[#f9f9f9] p-6 rounded-2xl border border-[rgba(0,0,0,0.05)]">
                ...
                <ul className="space-y-3">{localRssArticles.map((a, i) => (
                    <li key={i} className="flex flex-col gap-1 text-sm">
                      ...
                      <div className="text-xs text-[#717684] pl-6"><span className="uppercase font-semibold">{a.source}</span> • <a href={a.link} target="_blank" className="text-[#c0ae66] hover:underline">Artikel öffnen <ExternalLink className="w-3 h-3 inline"/></a></div>
                    </li>
                ))}</ul>
              </div>
            )}
             ...
             {['intro', 'outro'].map((type) => {
                 ...
                 return (
                    <div key={type} className={`rounded-2xl p-5 border ${file?'bg-[#fcfcfc] border-[#c0ae66]':'bg-[#f9f9f9]'}`}>
                        ...
                        <label className="block w-full bg-white border rounded-xl py-3 px-4 text-xs text-[#717684] mb-3 truncate cursor-pointer">{file?file.name:"Wähle MP3..."}<input type="file" accept="audio/*" className="hidden" onChange={(e)=>handleFileUpload(e, type as any)}/></label>
...
const App: React.FC = () => {
  ...
  return (
    <div className="container mx-auto max-w-[1200px] bg-white rounded-[16px] min-h-[calc(100vh-48px)] overflow-hidden shadow-sm border border-[rgba(0,0,0,0.05)]">
      ...
      <div className="text-center py-8 text-[#717684] text-xs bg-white border-t border-[rgba(0,0,0,0.05)]">Powered by Gemini 2.5 Flash & Flash-TTS • ANY EVER Audio Engine</div>
    </div>
  );
};
